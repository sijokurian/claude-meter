...
// SetIconByName sets the icon via IconName and IconThemePath D-Bus properties.
// Some Linux desktop environments (e.g. Ubuntu/GNOME) prefer IconName over IconPixmap.
func SetIconByName(iconName string, themePath string) {
	instance.lock.Lock()
	props := instance.props
	conn := instance.conn
	defer instance.lock.Unlock()
	...
	_ = notifier.Emit(conn, &notifier.StatusNotifierItem_NewIconSignal{
		Path: path,
		Body: &notifier.StatusNotifierItem_NewIconSignalBody{},
	})
}

// SetLabel sets the XAyatanaLabel D-Bus property, which shows text next to the
// tray icon at the panel's native font size on Ubuntu/GNOME.
func SetLabel(label string) {
	...
	props.SetMust("org.kde.StatusNotifierItem", "XAyatanaLabel", label)
	props.SetMust("org.kde.StatusNotifierItem", "XAyatanaLabelGuide", "000%")

	if conn == nil {
		return
	}

	_ = notifier.Emit(conn, &notifier.StatusNotifierItem_NewTitleSignal{
		Path: path,
